...
export const forceTitleNotation = (term: string) => {
	...
	//post-processing
	r = qstr.replaceAll(r, 'J S O N', 'JSON');
	r = qstr.replaceAll(r, 'a P I', 'API');
	r = qstr.replaceAll(r, 'S P a', 'SPA');
	r = qstr.replaceAll(r, 'J S X', 'JSX');
...
export const getSmartTitleFromUrl = (url: string) => {
	let r = ''
	if (url.includes('stackoverflow.com')) {
		// e.g. https://stackoverflow.com/questions/33445415/javascript-promises-reject-vs-throw
		const parts = qstr.breakIntoParts(url, '/');
		r = parts[parts.length - 1]; // e.g. javascript-promises-reject-vs-throw
		r = qstr.replaceAll(r, '-', ' '); // e.g. javascript promises reject vs throw
		r = qstr.forceTitleNotation(r); // e.g. Javascript Promises Reject vs. Throw
	}
	if (url.includes('developer.mozilla.org')) {
		// e.g. https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/catch
		const parts = qstr.breakIntoParts(url, '/');
		const mainWord = parts[parts.length - 2];
		const functionWord = parts[parts.length - 1];
		r = `${mainWord}.${functionWord}()`;
	}
	if (r === '') {
		return qstr.convertUrlToTitle(url);
	}
	return r;
}
...
// e.g. https://www.freecodecamp.org/news/react-under-the-hood/
export const convertUrlToTitle = (url: string) => {
	let r = url;

	r = qstr.chopLeft(r, 'https://');
	r = qstr.chopLeft(r, 'http://');
	r = qstr.chopRight(r, '/');
	r = qstr.chopRight(r, '.html');
	r = qstr.chopRight(r, '.php');
	r = qstr.chopRight(r, '.asp');

	const parts = qstr.breakIntoParts(r, '/');
	// const mainUrl = parts[0]; // e.g. www.freecodecamp.org
	parts.shift();
	r = parts.join('-'); // news-react-under-the-hood
